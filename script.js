...
function processMcq() {
    ...
    // Normalize Windows line endings to standard newlines
    text = text.replace(/\r\n/g, '\n');

    if (state.mcq.removeOpts) {
        // Remove option lines
        text = text.replace(/^\s*[A-Ea-e][\)\.].*$/gm, '');
        // Remove blank lines before "correct answer"
        text = text.replace(/\n\s*\n(?=(?:correct\s+)?answer[\s:])/gi, '\n');
        // Ensure exactly one blank line after "next question"
        text = text.replace(/next question\s*(\n+|$)/gi, 'next question\n\n');
        // Final cleanup: remove extra blank lines elsewhere
        text = text.replace(/\n{3,}/g, '\n\n');
        text = text.trim();
    }
...
function drawMCQImage() {
    ...
    let watermarkText = document.getElementById('img-watermark').value.trim() || "Generated by SM Tools";
